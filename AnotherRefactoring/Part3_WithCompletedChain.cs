...
/*
    The completed chain is an object oriente solution in which all the concerns have been
    separated to specific classes which can then be extended and composed according to the
    requirements. The original method just creates the chain of validators and delegates its
    check to them. The Christmas tree structure when building the chain is however a bit ugly
    and not really readable...
*/
[TestClass]
public class Refactored3
{
    ...
    public static bool IsValidDocument(string code)
    {
        ...
        {
            Next = new PaperIDCardValidator() 
            {
                Next = new PassportValidator()
                {
                    Next = new UcoOldDriversLicenceValidator() 
                    {
                        Next = new UcoDriversLicenceValidator() 
                        {
                            Next = new ProvinceDriversLicenceValidator()
                        }
                    } 
                } 
            } 
